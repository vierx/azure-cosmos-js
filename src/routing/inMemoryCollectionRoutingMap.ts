...
export class InMemoryCollectionRoutingMap {
    ...
    public getRangeByEffectivePartitionKey(effectivePartitionKeyValue: string) {
        if (
            Constants.EffectiveParitionKeyConstants
    ...
    public getOverlappingRanges(
        providedQueryRanges: QueryRange | QueryRange[]
    ) {
                ...
                if (queryRange.overlaps(this.orderedRanges[j])) {
                    minToPartitionRange[
                        this.orderedPartitionKeyRanges[j][
                            Constants.PartitionKeyRange.MinInclusive
                        ]
                    ] = this.orderedPartitionKeyRanges[j];
                }
            }
        ...
        const overlappingPartitionKeyRanges = Object.keys(
            minToPartitionRange
