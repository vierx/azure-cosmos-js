// tslint:disable:no-unecessary-class
// TODO: This should not be a class, but it is needed right now so we can monkey patch getAuthorizationTokenUsingMasterKey in the portal
...
export class AuthHandler {
  public async setAuthorizationHeader(
    ...
    if (authOptions.masterKey || authOptions.key) {
      ...
      this.getAuthorizationTokenUsingMasterKey(verb, resourceId, resourceType, headers, key);
    } else if (authOptions.resourceTokens) {
      headers[Constants.HttpHeaders.Authorization] = encodeURIComponent(
        this.getAuthorizationTokenUsingResourceTokens(authOptions.resourceTokens, path, resourceId)
      );
    } else if (authOptions.tokenProvider) {
      headers[Constants.HttpHeaders.Authorization] = encodeURIComponent(
        await this.getAuthorizationTokenUsingTokenProvider(authOptions.tokenProvider, {
          verb,
          path,
          resourceId,
          resourceType,
  ...
  private getAuthorizationTokenUsingMasterKey(
    verb: HTTPMethod,
    resourceId: string,
    resourceType: ResourceType,
    headers: IHeaders,
  ...
  private getAuthorizationTokenUsingResourceTokens(
  ...
  private getAuthorizationTokenUsingTokenProvider(
    tokenProvider: ITokenProvider,
    requestInfo: IRequestInfo
  ): Promise<string> {
    requestInfo.getAuthorizationTokenUsingMasterKey = this.getAuthorizationTokenUsingMasterKey;
