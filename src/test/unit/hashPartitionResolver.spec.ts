...
describe("HashPartitionResolver", function() {
    describe("new()", function() {
        it(" does not throw", function() {
            assert.doesNotThrow(function() {
                const resolver = new HashPartitionResolver("foo", [
                    "dbs/foo/colls/A"
                ]);
            });
        });
        ...
        it("invalid partitionKeyResolver", function() {
            assert.throws(function() {
                const resolver = new HashPartitionResolver(
                    undefined,
                    undefined
                );
    ...
    describe("#.resolveForRead", function() {
        it("valid key", function() {
            const resolver = new HashPartitionResolver("ignored", [
                "dbs/foo/colls/A",
                "dbs/foo/colls/B",
                "dbs/foo/colls/C"
            ]);
    ...
    describe("#._throwIfInvalidCollectionLinks", function() {
        it("non-array throws", function() {
            ...
            links.forEach(function(link) {
                assert.throws(function() {
                    HashPartitionResolver._throwIfInvalidCollectionLinks(
                        link as any
                    );
                }, /collectionLinks must be an array./);
            });
    ...
    describe("#._throwIfInvalidPartitionKeyExtractor", function() {
        ...
        it("null or undefined throws", function() {
            ...
            partitionKeyExtractors.forEach(function(
                partitionKeyExtractor: any
            ) {
                assert.throws(function() {
                    HashPartitionResolver._throwIfInvalidPartitionKeyExtractor(
    ...
    describe("#._throwIfInvalidPartitionKey", function() {
        ...
        it("throws", function() {
            ...
            keys.forEach(key => {
                assert.throws(() => {
                    HashPartitionResolver._throwIfInvalidPartitionKey(
                        key as any
                    );
                }, /partitionKey must be a 'string'/);
            });
