...
describe("InMemoryCollectionRoutingMap Tests", function() {
    describe("getOverlappingRanges", function() {
        ...
        it("queryCompleteRange", function() {
            ...
            assert.equal(
                overlappingPartitionKeyRanges.length,
                partitionKeyRanges.length
            );
            assert.deepEqual(overlappingPartitionKeyRanges, partitionKeyRanges);
        });

        it("queryEmptyRange", function() {
            const emtpyRange = new QueryRange(
                "05C1C9CD673396",
                "05C1C9CD673396",
                true,
                false
        ...
        it("queryPoint", function() {
            const pointRange = new QueryRange(
                "05C1D9CD673397",
                "05C1D9CD673397",
                true,
                true
            ...
            assert(
                overlappingPartitionKeyRanges[0].minInclusive <= pointRange.min
            );
            assert(
                overlappingPartitionKeyRanges[0].maxExclusive > pointRange.max
            );
        });
    ...
    describe("All methods", function() {
        ...
        it("validate _orderedPartitionKeyRanges", function() {
            assert.equal(
                "0",
                collectionRoutingMap.getOrderedParitionKeyRanges()[0].id
            );
            assert.equal(
                "1",
                collectionRoutingMap.getOrderedParitionKeyRanges()[1].id
            );
            assert.equal(
                "2",
                collectionRoutingMap.getOrderedParitionKeyRanges()[2].id
            );
            assert.equal(
                "3",
                collectionRoutingMap.getOrderedParitionKeyRanges()[3].id
            );
        });
        ...
        it("validate getRangeByEffectivePartitionKey", function() {
            assert.equal(
                "0",
                collectionRoutingMap.getRangeByEffectivePartitionKey("").id
            );
            assert.equal(
                "0",
                collectionRoutingMap.getRangeByEffectivePartitionKey(
                    "0000000000"
                ).id
            );
            assert.equal(
        ...
        it("validate getOverlappingRanges", function() {
            ...
            const overlappingRanges = _.sortBy(
            ...
            assert.equal(
                0,
                collectionRoutingMap.getOverlappingRanges([noPoint]).length
            );

            const onePoint = new QueryRange(
                "0000000040",
                "0000000040",
                true,
                true
            );
            let overlappingPartitionKeyRanges = collectionRoutingMap.getOverlappingRanges(
                [onePoint]
            );
            ...
            overlappingPartitionKeyRanges = _.sortBy(
                collectionRoutingMap.getOverlappingRanges(ranges),
                getKey
            );
