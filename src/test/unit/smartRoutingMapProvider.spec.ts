...
describe("Smart Routing Map Provider OverlappingRanges", function() {
    ...
    const mockedDocumentClient = new MockedDocumentClient(
        partitionKeyRanges,
        containerId
    );
    const smartRoutingMapProvider = new SmartRoutingMapProvider(
        mockedDocumentClient
    );
    const partitionKeyRangeCache = new PartitionKeyRangeCache(
        mockedDocumentClient
    );
    ...
    it("Empty Ranges are thrown away", async function() {
        ...
        try {
            await assertOverlappingRangesAreEqual(
                [e1, r1, e2, r2, e3, e4],
                [r1, r2]
            );
        } catch (err) {
    ...
    it("Complex", async function() {
        ...
        try {
            await validateOverlappingRanges(ranges, [
                partitionKeyRanges[1],
                partitionKeyRanges[4]
            ]);
        } catch (err) {
            throw err;
        }
    ...
    const validateOverlappingRanges = async function(
        ...
        try {
            ...
            await validateSmartOverlappingRanges(
                queryRanges,
                expectedResults,
                errorExpected
            );
