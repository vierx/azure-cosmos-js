...
describe("RangePartitionResolver", function() {
    ...
    describe("getFirstContainingMapEntryOrNull", function() {
        ...
        it("_tryGetContainingRange - map with no containing entry returns null", function(done) {
            ...
            const result = resolver.getFirstContainingMapEntryOrNull(
                new Range({ low: "B" })
            );
            assert.equal(result, null);
            done();
        ...
        it("_tryGetContainingRange - map with more multiple containing entries returns first entry", function(done) {
            ...
            const result1 = resolver1.getFirstContainingMapEntryOrNull(
                new Range({ low: "A" })
            );
            ...
            const result2 = resolver2.getFirstContainingMapEntryOrNull(
                new Range({ low: "A" })
            );
    ...
    const resolveForReadTest = function(
        resolver: any,
        partitionKey: any,
        expectedLinks: any
    ) {
