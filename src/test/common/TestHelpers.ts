...
export class TestHelpers {
    public static async removeAllDatabases(client: CosmosClient) {
        try {
            ...
            const count = 0;
            await Promise.all(
                databases.map<Promise<Response<DatabaseDefinition>>>(
                    async (database: DatabaseDefinition) =>
    ...
    public static async getTestDatabase(
        client: CosmosClient,
        testName: string
    ) {
    ...
    public static async bulkReadItems(
        ...
        for (const document of documents) {
            try {
                ...
                const { body: doc } = await container
                    .item(document.id)
                    .read(options);
        ...
        for (const document of documents) {
            try {
                const { body: doc } = await container
                    .item(document.id)
                    .replace(document);
                const expectedModifiedDocument = JSON.parse(
                ...
                assert.equal(
                    JSON.stringify(actualModifiedDocument),
                    JSON.stringify(expectedModifiedDocument)
                );
        ...
        for (const document of documents) {
            try {
                const options =
                    ...
                    document.hasOwnProperty(partitionKeyPropertyName)
