    ...
    describe("Newly updated documents should be fetched incremetally", function() {
        ...
        before(async function() {
            container = await TestHelpers.getTestContainer(
                client,
                "Newly updated documents should be fetched incrementally"
            );
        ...
        it("should fetch updated documents only", async function() {
            ...
            assert(
                headers.etag,
                "listDocuments response should have etag header"
            );
            ...
            assert.equal(
                results.length,
                2,
                "initial number of documents should be equal 2"
            );

            document.name = "xyz";

            const { body: replaced } = await container
                .item(document.id)
                .replace(document);
            assert.deepEqual(
                replaced.name,
                "xyz",
                "replaced document should be valid"
            );
            ...
            const { result: docs } = await container.items
                .readAll(options)
                .toArray();
            assert.equal(
                docs.length,
                1,
                "initial number of documents should be equal 1"
            );
            assert.equal(
                docs[0].name,
                "xyz",
                "fetched document should have 'name: xyz'"
            );
            assert.equal(
                docs[0].id,
                document.id,
                "fetched document should be valid"
            );
        });
    });
