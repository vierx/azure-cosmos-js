...
describe.skip("NodeJS Aggregate Query Tests", async function() {
    ...
    describe("Validate Aggregate Document Query", function() {
            ...
            try {
                while (results.length < expectedResults.length) {
                    ...
                    if (item === undefined) {
                        assert(
                            !queryIterator.hasMoreResults(),
                            "hasMoreResults must signal results exhausted"
                        );
                    ...
                    if (results.length < expectedResults.length) {
                        assert(
                            queryIterator.hasMoreResults(),
                            "hasMoreResults must indicate more results"
                        );
                    }
                }
            } catch (err) {
            ...
            try {
                while (results.length <= expectedResults.length) {
                    ...
                    assert.equal(
                        item,
                        currentItem,
                        "current must give the previously item returned by nextItem"
                    );
                }
                ...
                while (totalFetchedResults.length <= expectedResults.length) {
                    ...
                    if (totalFetchedResults.length < expectedResults.length) {
                        ...
                        assert(
                            ...
                            "executeNext: invalid fetch block size"
                        );
                        assert.equal(
                            results.length,
                            pageSize,
                        ...
                        assert(
                            queryIterator.hasMoreResults(),
                            "hasMoreResults expects to return true"
                        );
                    } else {
                        // no more results
                        assert.equal(
        ...
        const executeQueryAndValidateResults = async function(
            ...
            queryIterator.reset();
            await validateNextItemAndCurrentAndHasMoreResults(
                queryIterator,
                expectedResults
            );
        ...
        const generateTestConfigs = function() {
            ...
            const aggregateQueryFormat =
                "SELECT VALUE %s(r.%s) FROM r WHERE %s";
            const aggregateOrderByQueryFormat =
                "SELECT VALUE %s(r.%s) FROM r WHERE %s ORDER BY r.%s";
            const aggregateConfigs = [
                {
                    operator: "AVG",
                    expected:
                        testdata.sum / testdata.numberOfDocumentsWithNumbericId,
            ...
            const aggregateSinglePartitionQueryFormat =
                "SELECT VALUE %s(r.%s) FROM r WHERE r.%s = '%s'";
            const aggregateSinglePartitionQueryFormatSelect =
                "SELECT %s(r.%s) FROM r WHERE r.%s = '%s'";
        ...
        generateTestConfigs().forEach(function(test) {
            it(test.testName, async function() {
                try {
                    const expected =
                        test.expected === undefined ? [] : [test.expected];
