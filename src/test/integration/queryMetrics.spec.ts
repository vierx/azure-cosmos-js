...
describe("QueryMetrics", function() {
    ...
    const assertQueryMetricsEquality = function(
        ...
        assert.deepEqual(
            queryMetrics1.indexHitRatio,
            queryMetrics2.indexHitRatio
        );
        assert.deepEqual(
            queryMetrics1.outputDocumentCount,
            queryMetrics2.outputDocumentCount
        );
        assert.deepEqual(
            queryMetrics1.outputDocumentSize,
            queryMetrics2.outputDocumentSize
        );
        assert.deepEqual(
            queryMetrics1.retrievedDocumentCount,
            queryMetrics2.retrievedDocumentCount
        );
        assert.deepEqual(
            queryMetrics1.retrievedDocumentSize,
            queryMetrics2.retrievedDocumentSize
        );
        assert.deepEqual(
            queryMetrics1.totalQueryExecutionTime,
            queryMetrics2.totalQueryExecutionTime
        );

        assert.deepEqual(
            queryMetrics1.documentLoadTime,
            queryMetrics2.documentLoadTime
        );
        assert.deepEqual(
            queryMetrics1.documentWriteTime,
            queryMetrics2.documentWriteTime
        );
        assert.deepEqual(
            queryMetrics1.indexLookupTime,
            queryMetrics2.indexLookupTime
        );
        assert.deepEqual(
            queryMetrics1.vmExecutionTime,
            queryMetrics2.vmExecutionTime
        );
        ...
        assert.deepEqual(
            queryMetrics1.runtimeExecutionTimes
                .userDefinedFunctionExecutionTime,
    ...
    it("Should Add Two Query Metrics", function() {
        ...
        const doubleIndexHitCount =
            indexUtilizationRatio * retrievedDocumentCount * 2;
        ...
        const queryMetricsFromCreateArray = QueryMetrics.createFromArray([
            queryMetrics,
            queryMetrics
        ]);

        assertQueryMetricsEquality(
            queryMetricsFromCreateArray,
            expectedQueryMetrics
        );
    });
    ...
    it("Can Be Create From Delimited String", function() {
        ...
        assertQueryMetricsEquality(
            queryMetricsFromDelimitedString,
            queryMetrics
        );
    });
