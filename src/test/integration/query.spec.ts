...
describe("Test Query Metrics On Single Partition Collection", function() {
    ...
    const testQueryMetricsOnSinglePartition = async function(document: any) {
        try {
            ...
            const {
                body: createdCollectionDef
            } = await database.containers.create(
                collectionDefinition,
                collectionOptions
            );
            const createdContainer = database.container(
                createdCollectionDef.id
            );
            ...
            const queryIterator = createdContainer.items.query(
                query,
                queryOptions
            );
            ...
            while (queryIterator.hasMoreResults()) {
                ...
                assert.notEqual(
                    headers[Constants.HttpHeaders.QueryMetrics]["0"],
                    null
                );
            }
        } catch (err) {
