...
describe("Session Token", function() {
    ...
    it("validate session tokens for sequence of opearations", async function() {
        ...
        const { body: createdContainerDef } = await database.containers.create(
            containerDefinition,
            containerOptions
        );
        ...
        assert.equal(
            postSpy.lastCall.args[3][Constants.HttpHeaders.SessionToken],
            client.documentClient.sessionContainer.getCombinedSessionToken(
                tokens
            )
        );
        ...
        const { body: document13 } = await container.items.upsert(
        ...
        assert.equal(
            tokens[index1],
            (Number(firstPartitionLSN) + 1).toString()
        );
        assert.equal(tokens[index2], secondPartitionLSN);
        ...
        assert.equal(
            tokens[index2],
            (Number(secondPartitionLSN) + 1).toString()
        );
    ...
    it("validate 'lsn not caught up' error for higher lsn and clearing session token", async function() {
        const { body: databaseDef } = await client.databases.create(
            databaseBody
        );
                ...
                if (oldTokens.hasOwnProperty(coll)) {
                    for (const token in oldTokens[coll]) {
                        if (oldTokens[coll].hasOwnProperty(token)) {
                            const newVal = (
                                Number(oldTokens[coll][token]) + 2000
                            ).toString();
                            return token + ":" + newVal;
                        }
                    }
        ...
        const callbackSpy = sinon.spy(function(
            pat: string,
            reqHeaders: IHeaders
        ) {
            const oldTokens =
                client.documentClient.sessionContainer
                    .collectionResourceIdToSessionTokens;
            reqHeaders[Constants.HttpHeaders.SessionToken] = increaseLSN(
                oldTokens
            );
        });
        const applySessionTokenStub = sinon
            .stub(client.documentClient, "applySessionToken")
        ...
        try {
            ...
            assert.equal(
                err.substatus,
                1002,
                "Substatus should indicate the LSN didn't catchup."
            );
            assert.equal(callbackSpy.callCount, 1);
            assert.equal(
                Base._trimSlashes(callbackSpy.lastCall.args[0]),
                containerLink + "/docs/1"
            );
    ...
    it.skip("client should not have session token of a container created by another client", async function() {
        ...
        assert.notEqual(
            client2.documentClient.getSessionToken(container.url),
            ""
        );
    });
    ...
    it("validate session container update on 'Not found' with 'undefined' status code for non master resource", async function() {
        ...
        const { body: createdContainerDef } = await db.containers.create(
            containerDefinition,
            containerOptions
        );
        ...
        const setSessionTokenSpy = sinon.spy(
            client.documentClient.sessionContainer,
            "setSessionToken"
        );
        ...
        try {
            ...
            assert.equal(
                err.substatus,
                undefined,
                "expecting substatus code to be undefined"
            );
            assert.equal(
                setSessionTokenSpy.callCount,
                1,
                "unexpected number of calls to sesSessionToken"
            );
