...
export class RetryUtility {
    ...
    public static async execute(
        ...
        const endpointDiscoveryRetryPolicy = new EndpointDiscoveryRetryPolicy(
        ...
        const sessionReadRetryPolicy = new SessionReadRetryPolicy(
            globalEndpointManager,
            r
        );
    ...
    public static async apply(
        ...
        // TODO: any response
        const httpsRequest = createRequestObjectFunc(
            connectionPolicy,
            requestOptions,
            body
        );
        ...
        try {
            const { result, headers } = await (httpsRequest as Promise<
                Response<any>
            >);
                ...
                return new Promise<Response<any>>((resolve, reject) => {
                    setTimeout(async () => {
                        if (typeof newUrl !== "undefined") {
                            requestOptions = this.modifyRequestOptions(
                                requestOptions,
                                newUrl
                            );
                        }
    ...
    public static modifyRequestOptions(
        ...
        for (const index in properties) {
            if (properties[index] !== "path") {
                oldRequestOptions[properties[index]] =
                    newUrl[properties[index]];
            }
        }
